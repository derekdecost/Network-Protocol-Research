...
class SelectiveRepeat:
    ...
    def __init__(self, send_address, send_port, recv_address, recv_port, window_size, mss=500,corruption=0, corruption_option=[1, 2, 3], loss=0, loss_option=[1, 2, 3], timeout=None):
        ...
        self._ack_pending_buffer_l  = threading.Lock()
    ...
    def send(self, data):
        ...
        if DEBUG:
            print(f"SR: Starting ACK Receiving Process.")
        recv_ack_t.start()

        if DEBUG:
            print(f"SR: Starting Sending Process.")
        send_t.start()
    ...
    def recv(self):
        ...
        while self.base < total_packets:
            ...
            if verify_checksum(packet) and ((not ((packet_corrupted(self.corruption)) and (3 in self.corruption_option))) or (1 in self.corruption_option)):
                ...
                self._send_ack(header, total_packets)
            else:
                if DEBUG:
                    print(f"SR: Checksum Invalid.")
                continue

            # Create the data buffer based on the number of packets to be received from the sending process.
            if data_buffer is None:
                data_buffer = [None] * total_packets

            # Add non-buffered received data to the data buffer.
            if data_buffer[header] is None:
                data_buffer[header] = rcvd_data

            # Set the base value based on the lowest index of non-buffered data.
            try:
                self.base = data_buffer.index(None)
            except:
    ...
    def _send(self, data):
        ...
        while True:
            ...
            if self._send_complete_f:
                if DEBUG:
                    print("SR: Connection closed by remote host.")
                break
            ...
            # sending window.
            while seqnum < window_end:
                packet = self._add_header(data[seqnum], seqnum, len(data))
                ...
                self._ack_pending_buffer_l.acquire()
                self._ack_pending_buffer.append(threading.Timer(self.timeout, self._timeout, (packet, 0,)))
                self._ack_pending_buffer[seqnum].start()
                self._ack_pending_buffer_l.release()

                if DEBUG:
                    print(f"SR: Sending Packet {seqnum}/{len(data) - 1}")
                else:
                    print(f"SR: Sending Packet {seqnum}/{len(data) - 1}", end="\r")
        ...
        if DEBUG:
            print(f"SR: Data transfer complete.")
    ...
    def _recv_ack(self):
        ...
        while True:
            ...
            header, total_data, rcvd_data, cs  = self._parse_packet(packet)
            header                  = int.from_bytes(header, 'big')     # Sequence number.
            total_data              = int.from_bytes(total_data, 'big') # Total number of packets in the transfer.
            ...
            if not verify_checksum(packet) or (packet_corrupted(self.corruption) and (2 in self.corruption_option)):
                if DEBUG:
                    print(f"SR: Checksum invalid.")
                continue

            # Create the ACK buffer based on the amount of packets to be transferred to the receiving host.
            if ack_buffer is None:
                ack_buffer = [False] * total_data

            # Buffer received ACKs.
            ack_buffer[header] = True

            # Stop the timeout process associated with the received ACK.
            self._ack_pending_buffer_l.acquire()
            if DEBUG:
                print(f"SR: ACK{header} received.")
            self._ack_pending_buffer[header].cancel()
            self._ack_pending_buffer_l.release()

            # Update the base value based on the sequence number of the received ACK.
            self._base_l.acquire()
            try:
                self.base = ack_buffer.index(False)
            except:
                self.base = total_data
            if DEBUG:
                print(f"SR: Base increase to {self.base}")
            self._base_l.release()
    ...
    #
    # @param    packet  - ByteArray object containing data to be sent to a remote host.
    # @param    retry   - integer representing the current send retry value.
    #
    # @return   None.
    def _timeout(self, packet, retry):
        ...
        if DEBUG:
            print(f"SR: ACK{header} receive timed out. Resending packet {header}.")
        if packet_lost(self.loss) and (3 in self.loss_option) and (not 1 in self.loss_option):
            pass
        else:
            self._send_l.acquire()
        ...
        # Reset the timout thread in the ACK pending buffer.
        self._ack_pending_buffer_l.acquire()
        self._ack_pending_buffer[header] = threading.Timer(self.timeout, self._timeout, (packet, retry_cnt,))
        self._ack_pending_buffer[header].start()
        self._ack_pending_buffer_l.release()
        return
    ...
    def _send_ack(self, state, total_packets):
        ...
        if DEBUG:
            print(f"SR: Sending ACK{state}")
        else:
            print(f"SR: Sending ACK{state}", end="\r")

        if packet_lost(self.loss) and (2 in self.loss_option):
            return
